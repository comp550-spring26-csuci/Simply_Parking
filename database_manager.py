...
#                database=os.getenv("DB_NAME"),
            ...
            if self.conn.is_connected():
                ...
                self._init_semester_permits_table()
                self._init_daily_permits_table()
    ...
    def _init_semester_permits_table(self): # Added for semester permit management (AT)
        cur = self.conn.cursor()
        try:
            cur.execute(
                """
    ...
    def _init_daily_permits_table(self):
        cur = self.conn.cursor()
        try:
            cur.execute(
                """
    ...
    def create_semester_permit(self, user_id: int, plate: str, semester: str, year: str) -> bool:
        plate = (plate or "").strip().upper()
        semester = (semester or "").strip()
        year = (year or "").strip()

        if not user_id or not plate or not semester or not year:
            return False
        ...
        try:
            cur.execute(
                """
                INSERT INTO semester_permits (user_id, plate, semester, year, status, created_at)
                VALUES (%s, %s, %s, %s, %s, %s)
                """,
                (user_id, plate, semester, year, "Active", self._now_str()),
            )
            self.conn.commit()
            return True
        except Error as e:
    ...
    def fetch_semester_permits_by_user(self, user_id: int): # Fetch semester permits for a specific user (AT)
        cur = self.conn.cursor()
        try:
            cur.execute(
                """
                SELECT id, plate, semester, year, status, created_at
    ...
    def fetch_daily_permits_by_user(self, user_id: int): # Fetch daily permits for a specific user (AT)
        cur = self.conn.cursor()
        try:
            cur.execute(
                """
                SELECT id, plate, permit_date, status, created_at
                FROM daily_permits
                WHERE user_id = %s
