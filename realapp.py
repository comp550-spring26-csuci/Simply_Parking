...
print(database_manager.__file__)
from database_manager import DatabaseManager

print("DatabaseManager file:", database_manager.__file__)
print("DatabaseManager PAYG methods:", [m for m in dir(DatabaseManager) if "session" in m.lower()])
...
class PlateApp:
    def __init__(self, root):
        ...
        try:
           ...
           # print("Has create_daily_permit:", hasattr(self.db, "create_daily_permit")) # Debug line to check if the method exists in the DatabaseManager instance
    ...
    def show_dashboard(self):
        ...
        if role == "admin":
            ...
            self.add_nav_button(nav, "Buy Semester Permit", self.show_buy_semester_permit) # Added for semester users to purchase a new permit (AT)
            self.add_nav_button(nav, "My Semester Permit", self.show_my_semester_permit) # Added for semester users to view their permit details (AT)
    ...
    def show_buy_semester_permit(self): #semester permit purchase view for users to buy a new semester permit (AT)
        self.clear_content()

        frame = tk.Frame(self.content_frame, padx=20, pady=20)
        frame.pack(anchor="nw")

        tk.Label(frame, text="Buy Semester Permit", font=("Arial", 16, "bold")).grid(row=0, column=0, columnspan=2, pady=10)

        tk.Label(frame, text="Semester").grid(row=1, column=0, sticky="e", pady=5)
        ...
        semester_combo.grid(row=1, column=1, pady=5)
        semester_combo.set("Spring")

        tk.Label(frame, text="Year").grid(row=2, column=0, sticky="e", pady=5)
        year_entry = tk.Entry(frame, width=30)
        year_entry.grid(row=2, column=1, pady=5)

        tk.Label(frame, text="Vehicle Plate").grid(row=3, column=0, sticky="e", pady=5)
        plate_entry = tk.Entry(frame, width=30)
        plate_entry.grid(row=3, column=1, pady=5)

        def save_semester_permit():
            ...
            if not semester or not year or not plate:
                messagebox.showwarning("Input Error", "Semester, year, and vehicle plate are required.")
                return

            ok = self.db.create_semester_permit(
                user_id=self.current_user["id"],
                plate=plate,
                semester=semester,
                year=year,
            ...
            print("create_semester_permit returned:", ok)

            if ok:
                self.db.add_log(
                    event_type="semester_permit_created",
                    details=f"Plate={plate}, semester={semester}, year={year}",
                    user_id=self.current_user["id"],
                    username=self.current_user["username"],
                )
                messagebox.showinfo("Success", "Semester permit created.")
                self.show_my_semester_permit()
            else:
                messagebox.showerror("Error", "Could not create semester permit.")

        tk.Button(frame, text="Buy Permit", command=save_semester_permit).grid(
            row=4, column=0, columnspan=2, pady=10
        )

    def show_my_semester_permit(self): #semester permit details view for users to see their current permit status and details (AT)
        self.clear_content()

        top = tk.Frame(self.content_frame, padx=10, pady=10)
        top.pack(fill="x")

        tk.Label(top, text="My Semester Permit", font=("Arial", 16, "bold")).pack(side="left")
        tk.Button(top, text="Refresh", command=self.show_my_semester_permit).pack(side="right")

        table_frame = tk.Frame(self.content_frame, padx=10, pady=10)
        table_frame.pack(fill="both", expand=True)

        columns = ("id", "plate", "semester", "year", "status", "created_at")
        tree = ttk.Treeview(table_frame, columns=columns, show="headings")
        ...
        try:
            rows = self.db.fetch_semester_permits_by_user(self.current_user["id"])
            for row in rows:
                tree.insert("", tk.END, values=row)
        except AttributeError:
            messagebox.showerror(
                "Missing Database Method",
                "DatabaseManager.fetch_semester_permits_by_user() is not implemented yet."
            )
        except Exception as e:
            messagebox.showerror("Error", str(e))  
    ...
    #def show_buy_daily_permit(self): Replace with method below to show daily permit purchase form (AT)
    #    self.clear_content()
    ...
    def show_buy_daily_permit(self):
        ...
        tk.Label(frame, text="Buy Daily Permit", font=("Arial", 16, "bold")).grid(row=0, column=0, columnspan=2, pady=10)

        tk.Label(frame, text="Permit Date (YYYY-MM-DD)").grid(row=1, column=0, sticky="e", pady=5)
        date_entry = tk.Entry(frame, width=30)
        date_entry.grid(row=1, column=1, pady=5)

        tk.Label(frame, text="Vehicle Plate").grid(row=2, column=0, sticky="e", pady=5)
        plate_entry = tk.Entry(frame, width=30)
        plate_entry.grid(row=2, column=1, pady=5)

        def save_daily_permit():
            permit_date = date_entry.get().strip()
            plate = plate_entry.get().strip()

            print("Button Clicked")
            print("Permit Date:", permit_date)
            print("Plate:", plate)
            print("User ID:", self.current_user["id"])

            if not permit_date or not plate:
                messagebox.showwarning("Input Error", "Permit date and plate are required.")
                return

            ok = self.db.create_daily_permit(
                user_id=self.current_user["id"],
                plate=plate,
            ...
            if ok:
                self.db.add_log(
                    event_type="daily_permit_created",
                    details=f"Plate={plate}, permit_date={permit_date}",
                    user_id=self.current_user["id"],
                    username=self.current_user["username"],
                )
                messagebox.showinfo("Success", "Daily permit created.")
                self.show_my_daily_permit()
            else:
                messagebox.showerror("Error", "Could not create daily permit.")

        tk.Button(frame, text="Buy Permit", command=save_daily_permit).grid(
            row=3, column=0, columnspan=2, pady=10
        )

    #def show_my_daily_permit(self): Replace with method below to show user's current daily permit details (AT)
    #    self.clear_content()
    ...
    def show_my_daily_permit(self):
        ...
        tk.Label(top, text="My Daily Permits", font=("Arial", 16, "bold")).pack(side="left")
        tk.Button(top, text="Refresh", command=self.show_my_daily_permit).pack(side="right")

        table_frame = tk.Frame(self.content_frame, padx=10, pady=10)
        table_frame.pack(fill="both", expand=True)

        columns = ("id", "plate", "permit_date", "status", "created_at")
        tree = ttk.Treeview(table_frame, columns=columns, show="headings")
        ...
        tree.column("id", width=60, anchor="center")
        tree.column("plate", width=120, anchor="center")
        tree.column("permit_date", width=140, anchor="center")
        tree.column("status", width=120, anchor="center")
        tree.column("created_at", width=180, anchor="center")
        ...
        try:
            rows = self.db.fetch_daily_permits_by_user(self.current_user["id"])
            for row in rows:
                tree.insert("", tk.END, values=row)
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def show_current_session(self):
        ...
        session = self.db.get_active_session(self.current_user["id"])

        if session:
            session_id, plate, start_time = session

            tk.Label(frame, text="Active Session").grid(row=1, column=0, columnspan=2, pady=10)
            tk.Label(frame, text=f"Plate: {plate}").grid(row=2, column=0, columnspan=2, pady=5)
            tk.Label(frame, text=f"Started At: {start_time}").grid(row=3, column=0, columnspan=2, pady=5)

        else:
            tk.Label(frame, text="No active session").grid(row=1, column=0, columnspan=2, pady=10)

            tk.Label(frame, text="License Plate").grid(row=2, column=0, sticky="e", pady=5)
            plate_entry = tk.Entry(frame, width=30)
            plate_entry.grid(row=2, column=1, pady=5)
            ...
            def start():
                ...
                ok = self.db.start_session(self.current_user["id"], plate)

                if ok:
                    messagebox.showinfo("Success", "Parking session started.")
                    self.show_current_session()
                else:
                    messagebox.showerror("Error", "Could not start session.")

            tk.Button(frame, text="Start Session", command=start).grid(
                row=3, column=0, columnspan=2, pady=10
            )
    ...
    def show_pay_exit(self):
        ...
        if not session:
            tk.Label(frame, text="No active parking session found.").grid(row=1, column=0, columnspan=2, pady=10)
            return

        session_id, plate, start_time = session

        tk.Label(frame, text=f"Plate: {plate}").grid(row=1, column=0, columnspan=2, pady=5)
        tk.Label(frame, text=f"Started At: {start_time}").grid(row=2, column=0, columnspan=2, pady=5)
        tk.Label(frame, text="Rule: Free for 30 minutes, then $6 daily charge.").grid(row=3, column=0, columnspan=2, pady=10)

        def pay():
            amount = self.db.end_session(self.current_user["id"])

            if amount is None:
                messagebox.showwarning("No Session", "No active session found.")
            elif float(amount) == 0:
                messagebox.showinfo("Exit", "You left within 30 minutes. No charge.")
            else:
                messagebox.showinfo("Payment", f"Daily charge applied: ${amount}")

            self.show_current_session()

        tk.Button(frame, text="End Session & Pay", command=pay).grid(
            row=4, column=0, columnspan=2, pady=10
        )
